package com.zerotoheroes.hsgameparser.achievements;

public class GlobalStats {

    public static class Key {
        public static final String TOTAL_DAMAGE_TO_ENEMY_HERO = "total-damage-to-enemy-hero";
        public static final String TOTAL_MANA_SPENT = "total-mana-spent";
        public static final String TOTAL_ENEMY_MINIONS_DEATH = "total-enemy-minions-death";
        public static final String TOTAL_DURATION = "total-duration";
        public static final String TOTAL_NUMBER_OF_MATCHES = "total-number-of-matches";
        public static final String TOTAL_TAVERN_UPGRADES = "total-tavern-upgrades";
        public static final String TOTAL_TAVERN_LOCK_ALL = "total-tavern-lock-all";
        public static final String TOTAL_TAVERN_REROLL = "total-tavern-reroll";
        public static final String TOTAL_ENEMY_HEROES_KILLED = "total-enemy-heroes-killed";
        public static final String BEST_RANK = "best-rank";
    }

    public static class Context {
        public static final String GLOBAL = "global";
        public static final String BATTLEGROUNDS = "battlegrounds";

    }
}
